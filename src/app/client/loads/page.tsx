...
function getStatusBadge(status: Load['status']) {
  switch (status) {
    ...
    case 'in_bidding':
      return <span className="px-2 py-1 bg-orange-100 text-orange-800 text-xs font-medium rounded-full">In Bidding</span>
    case 'assigned':
      ...
      {load.assignedDriver && (
        <div className="mb-4 p-3 bg-orange-50 rounded-lg">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-orange-900">Assigned to: {load.assignedDriver}</span>
          </div>
        </div>
      )}
