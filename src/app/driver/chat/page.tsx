...
function NewChatModal({ 
  ...
  const handleSubmit = () => {
    if (selectedClient && jobReference.trim()) {
      // Generate a proper UUID for the job_id
      const jobUUID = generateUUID()
      onStartChat(selectedClient, jobUUID)
      setSelectedClient('')
      setJobReference('')
      setSearchTerm('')
      onClose()
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
                  ...
                  filteredClients.map(client => (
                    <div
                      ...
                      className={`p-3 flex items-center space-x-3 cursor-pointer hover:bg-orange-50 border-b border-stone-100 last:border-b-0 ${
                        selectedClient === client.id ? 'bg-orange-50 border-orange-200' : ''
                      }`}
                    >
          ...
          <button
            ...
            disabled={!selectedClient || !jobReference.trim()}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="p-2.5 rounded-full bg-amber-100 text-amber-600 hover:bg-amber-200 transition-colors"
              title="Video Call"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function DriverChat() {
  ...
  const handleStartNewChat = async (clientId: string, jobId: string) => {
    try {
      if (!user?.id) {
      ...
      console.log('Starting chat with:', { clientId, driverId: user.id, jobId })
      const conversation = await messageService.getOrCreateConversation(clientId, user.id, jobId)
  ...
  const handleStartCall = async (callType: 'audio' | 'video') => {
    ...
    try {
      const { callId, localStream } = await callService.initiateCall(
        user.id,
      ...
      setCurrentCall({
        id: callId,
        caller_id: user.id,
        callee_id: selectedConv.otherUserId,
        ...
        <div className="page-header">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="page-subtitle">Communicate with your clients in real-time</p>
            </div>
          </div>
        </div>
